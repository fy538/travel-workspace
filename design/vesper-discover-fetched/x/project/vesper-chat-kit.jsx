...
function UserBubble({ who, time, children }) {
  const p = PEOPLE[who];
  // The current user ("You") mirrors to the right; everyone else stays left.
  const me = who === 'tiger' || p.name === 'You';
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexDirection: me ? 'row-reverse' : 'row' }}>
      <Avatar who={who}/>
      <div style={{ flex: 1, textAlign: me ? 'right' : 'left' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 5, justifyContent: me ? 'flex-end' : 'flex-start' }}>
          {me && <span style={{ fontFamily: T.mono, fontSize: 9, color: T.muteSoft, letterSpacing: 1 }}>{time}</span>}
          <span style={{ fontSize: 12, fontWeight: 600, color: T.ink }}>{p.name}</span>
          {!me && <span style={{ fontFamily: T.mono, fontSize: 9, color: T.muteSoft, letterSpacing: 1 }}>{time}</span>}
        </div>
        <div style={{
          display: 'inline-block',
          background: me ? T.ink : 'rgba(27,23,20,0.05)',
          color: me ? T.cardWarm : T.inkSoft,
          borderRadius: me ? '14px 4px 14px 14px' : '4px 14px 14px 14px',
          padding: '8px 12px', fontFamily: T.serif, fontSize: 13.5, lineHeight: 1.35,
        }}>{children}</div>
      </div>
    </div>
  );
