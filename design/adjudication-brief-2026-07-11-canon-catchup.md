...
**`CostsBalanceSheet` migrated — decided sans.** Flagged the serif-vs-sans mismatch rather than forcing it silently; asked, and the call came back to convert. `CostsBalanceSheet.tsx` now uses `SheetHeader size="dense"` for its title (dropping the hand-rolled `serif_semibold` 17px in favor of canon's sans spec), keeping the existing "Done" text-link via `SheetHeader`'s `right` slot override rather than switching to the default bare close-X (that structural question wasn't in scope — only the font-family/size mismatch was). D21 is now fully closed end-to-end: canon ruling → code size ladder → the one migration it was blocking.
