...
entity-health: ## Print content-free canonical entity queues, redirects, and stale-claim counts
	@cd travel-agent && PYTHONPATH=. .venv/bin/python scripts/entity_health_report.py
...
journey-evidence-promote: ## Promote current clean pass receipts into the committed attestation index
	@python3 scripts/promote_journey_evidence.py --write
...
m0-reachability-report: ## Run and report the five M0 authority/contract gates
	@python3 scripts/m0_reachability_report.py
...
life-composition-fixture-check: ## Validate the frozen Life composition dataset for visual and native labs
	@python3 scripts/check_life_composition_fixture.py

life-engine-fixture-check: ## Validate the W1-W6 Life organization replay manifest
	@python3 scripts/check_life_engine_fixture.py
