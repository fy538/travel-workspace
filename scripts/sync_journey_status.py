...
_EV_BEGIN = "<!-- BEGIN auto:evidence-attestations (generated by scripts/sync_journey_status.py — DO NOT EDIT) -->"
_EV_END = "<!-- END auto:evidence-attestations -->"
...
def build_evidence_block() -> str:
    ...
    if not attestations:
        rows.append("| — | — | ○ unrun | no promoted receipt |")
    else:
        for attestation in attestations:
            receipt = attestation.get("receipt", {})
            result = "✅ pass" if current_candidate else "STALE"
            receipt_ref = str(attestation.get("receipt_sha256", "—"))[:19]
            for journey in receipt.get("journeys", ["—"]):
                rows.append(
                    f"| {journey} | {receipt.get('layer', '—')} | {result} | `{receipt_ref}…` |"
                )
    candidate_label = "not promoted"
    if candidate:
        candidate_label = " / ".join(
            str(candidate.get(key, "unknown"))[:12]
...
def main() -> int:
    ...
    blocks = [
        ...
        ("evidence-attestations", build_evidence_block(), _EV_BEGIN, _EV_END),
    ]
