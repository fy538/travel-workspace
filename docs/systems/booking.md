...
- **Cancellation recovery survives restart** — public booking reads preserve the exact viewer-safe lifecycle (`requested`, `processing`, `pending_verification`, `refreshing`, `refresh_failed`, `cancelled`, `not_cancelled`, or `manual_action_required`) plus controller, checked/requested timestamps, active-booking posture, changed/unchanged facts, and a typed next action. Raw provider reconciliation evidence remains private.
- **Cancellation completion is delivered exactly once** — background terminal outcomes queue a retryable receipt lease, and a durable message idempotency key prevents duplicate trip-conversation notifications across crashes.
- **Cancelled-booking Costs review is system-authored** — confirmed cancellation pauses only the linked booking expense from settlement without changing its amount or fabricating a human opener. The payer or organizer resumes it after reviewing refund/fee evidence; any live recorded settlement payment must be voided first.
- **Manual cancellation review is operator-only** — exhausted cancellation claims appear oldest-first at the admin-token-gated cancellation review queue. The projection contains only bounded provider references, error evidence, and timestamps; it excludes arbitrary provider payloads and cannot mutate provider state.
- **Cancellation review resolution preserves both outcomes** — an operator resolution is correlation-bound, evidence-backed, idempotent, and atomic with its audit event. Provider-confirmed cancellation commits cancelled truth; provider-confirmed active truth keeps the booking confirmed and queues a durable user update before a new cancellation claim can be submitted.
...
- **Provider evidence follows booking-controller authority** — all trip members can read shared offer/plan status and controller attribution, but only the current booking controller receives provider references, passenger/payment evidence, deep links, or continuation capability. Organizer status alone does not broaden that booking-session authority.
...
- Cancellation retry budget exhausted → retain locally confirmed booking truth and enqueue the claim for operator provider-record review; the queue itself is read-only and never repeats cancellation.
- Operator proves the booking remains active → close the manual review as `not_cancelled`, notify the traveler durably, then archive that claim only when a later explicit cancellation creates a new claim.
