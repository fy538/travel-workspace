...
- **Inbound writes:** manual add-expense (FE sheet), receipt-derived expense, chat-initiated `log_expense`/`settle_expense_shares` (concierge tool_handlers), and the accommodation-cost sync (below) — every path validates `paid_by` and every split `user_id` against trip membership before a money row is written.
...
- **OPT-IN ONLY, with one scoped exception.** General booking/provider-receipt auto-logging (`auto_log.py`) was removed 2026-06-26 and must not be reintroduced — sharing a booking total requires organizer opt-in (journey 10/12 "must never happen"). The **accommodation-cost sync** (`accommodation_cost.py`, `source="accommodation_auto"`) is a narrow, intentional exception scoped only to stays: it keeps the Stay ↔ Costs relationship exact (`trip_accommodation_id` link) and is always visible/editable, never masked by default. Do not generalize this exception to other booking categories.
...
- **Idempotent creation** — repeated add-expense / record-payment calls (e.g. a retried client request) do not double-write, via `idempotency_key`.
...
- No proof that the booking → opt-in-share → ledger-entry chain preserves `paid_by` / split identity across the repo seam (journey 10 audit flagged it; unchanged since).
