...
- **Trip-member and evidence scoping are mandatory** — `paid_by` and every split user must be trip members, or no row is written (guards financial identity spoofing, journey 10 seam check). A generic expense may attach a receipt only when that receipt belongs to the same path trip; the API fails closed with an opaque 404 and PostgreSQL enforces the `(receipt_id, trip_id)` relationship for every writer. This applies uniformly across the HTTP routes, receipt-derived creation, and the concierge tool handlers.
- **An existing expense's `split_type` is immutable.** There is no truthful way to rewrite the historical allocation grammar in place. `ExpenseUpdate` does not expose the field and forbids extra fields, the mock API rejects stale runtime calls, and the edit sheet renders split type as display-only. Correct a wrong split grammar by replacing the expense, not by mutating its identity.
- **Review actions are projected, then re-authorized.** Every expense read carries the current viewer's exact ability to open, withdraw, or resolve a review plus denial/recovery posture. Only the payer or an assigned traveler may open; only the opener may withdraw; only the payer or trip organizer may resolve. A live payment makes review opening void-first. These read capabilities drive the UI but never replace transactional write checks.
