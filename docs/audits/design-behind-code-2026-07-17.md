...
5. **P1 — Member-initiated nudges.** Real nudge endpoint from Changes ("Nudge" on Waiting-on rows; reasons proposal/stay-vote/settle-up; transactional cap signal; unmount-safe deferral). Canon has `MissingVoterLine` but no nudge affordance/cooldown/sent/recipient rendering. → Add the nudge family to decision grammar.
