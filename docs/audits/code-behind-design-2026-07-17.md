...
### 1.6 Trip lifecycle recovery is unbuilt — P1

The workflow canon specifies readable cancelled records, Recover, Recover from archive, Reuse as template, and provider revalidation. Production currently supports Archive only; no trip-level cancel, unarchive/recover, or reuse-template API/UI was found.

Do not substitute a cosmetic hard-delete button. The valid target is durable, readable lifecycle state and explicit recovery.
...
5. Cancel/recover/unarchive/reuse-template lifecycle.
