...
| M12 | Receipt/OCR scan states | Complete confirm-before-add loading, unreadable, partial, retry, and correction states | Scan fixtures and accessibility pass | DESIGN VALIDATED |
...
- Code-parity Wave 2 (2026-07-18): M04–M05 viewer-safe offer projection, controller-only provider continuation, complete named consent ledger, reminder evidence, and controller/included/excluded/viewer action matrices are implemented with API, PostgreSQL consent, frontend, and generated-contract evidence. They remain below `PARITY VALIDATED` pending accepted device and accessibility certification.
- Code-parity Wave 3 (2026-07-18): M06 and M10 are implementation-complete through server-authored expense-review capabilities and an immutable update-time split contract. M07–M09 passed the backend expense/PostgreSQL and app Costs evidence families, including payment void/restoration, currency truth, and masked-view guards. All remain below `PARITY VALIDATED` pending the accepted real-device/accessibility matrix and explicit real-backend mixed-currency/masked-view walk.
- Code-parity Wave 4 (2026-07-18): M11 archive readiness now uses one read/transaction classifier, refuses every active booking session and provider hold with an exact destination, reconciles read-to-write races in Trip Settings, preserves archive/create-session lock ordering, and renders archived booking history without forward actions while retaining valid provider recovery. M11 is implementation-complete but remains below `PARITY VALIDATED` pending accepted real-device/accessibility evidence.
- Code-parity Wave 5 (2026-07-18): M12 now has typed OCR result/status/quality truth, explicit complete/partial/unreadable/manual client states, race-safe manual supersession, field-level suggestion/correction handling, retained receipts through OCR failure and manual continuation, idempotent save recovery, protected receipt visibility, and an exact canonical Costs return. M12 is implementation-complete but remains below `PARITY VALIDATED` pending accepted device/accessibility capture evidence.
- Code-parity Wave 6 (2026-07-18): Story, Invite, Proposal, and Unpacked now share one default-deny public-projection registry; Invite public payloads were minimized, Story/Unpacked gained actor controls and expiry, Proposal remains a generic authenticated handoff, and unavailable states no longer act as lifecycle oracles. S01–S02 remain below `PARITY VALIDATED` pending complete device/card and accessibility evidence.
- Code-parity Wave 7 (2026-07-18): the persona/clock capture harness was repaired; current Booking, Costs, and External Sharing design receipts were registered; cancellation-pending and mixed participant-consent device states passed; all five registered Costs states passed; both registered External Sharing states passed; 340 focused backend tests passed, including 19 explicit PostgreSQL lifecycle tests. Device inspection also fixed contradictory confirmed-booking hero copy and a missing completed-trip roster fixture. No row is promoted yet: Receipt/OCR and archive state breadth, real-backend actor walks, forced network/public-card states, full Booking aggregation, and large-text/VoiceOver evidence remain open. See `wave-7-parity-evidence-and-promotion-2026-07-18.md`.
...
- Nineteenth design return: upload truth, long-scan supersession, partial-item copy, durability language, and the ledger destination were corrected, but M12 remains `DESIGNING`. Two new contrast findings remain; the requested local wrapper removal was not completed; canonical `VBtn`/`VField` receive an incompatible raw Costs palette; and the success state bypasses the canonical Costs header/expense-row grammar.
- Twentieth design return: Receipt/OCR source integration passed. The local button/field forks are gone, shared primitives receive a complete scoped palette adapter, meaningful text is back on readable tokens, the audit returned to its established baseline, and successful Add returns through the canonical Costs header and `ExpenseRow`. M12 promoted to `DESIGN VALIDATED`; all Round 1 design rows are now closed.
- Next gate: begin the Round 1 implementation/parity wave. Fixtures, route/mutation behavior, role and visibility assertions, accessibility checks, and production-source evidence are still required before any row becomes `PARITY VALIDATED`.
...
## Round 1 validation — nineteenth design return (Receipt/OCR correction return)

### Corrections that passed

- Determinate `62%` upload progress and its progress bar were removed; Uploading is now indeterminate.
- Long-scan copy now consistently says waiting keeps the scan active and manual continuation supersedes it; late results are ignored.
- The guessed `~5` line-item count is gone.
- The matrix now distinguishes server-durable receipt/OCR/expense truth from local unsaved reviewed fields.
- Successful Add now returns to a Costs-ledger composition rather than automatically opening Expense Detail.
- Four of the six new meaningful-text audit findings were corrected.

### Remaining blockers

1. The audit is still at 38 rather than the established 36 baseline. `OR ENTER IT MANUALLY` and the struck-through original OCR amount still use `K.muteSoft`; both are meaningful and must use a readable token.
2. `ScanPrimary`, `ScanSecondary`, and `RevField` still exist even though the prompt's completion gate explicitly required their removal. They are thinner wrappers now, but the validation should not claim the requested source cleanup is complete while those local adapters remain. Render canonical `VBtn` and `VField` directly at the affected call sites, or document a truly trivial no-geometry adapter only if unavoidable.
3. The canonical primitives are passed `p={K}`, but `K` does not satisfy their palette contracts. `VBtn` ghost needs `hairDark`; `VField` needs `sheet`, `hairDark`, `oxblood`, `amberBg`, `amberBd`, `steel`, and `sage`. These resolve to undefined, so ghost borders, ordinary field surfaces, suggestion hints, and missing-field error styling are invalid. Create a scoped palette adapter that maps existing Costs tokens to the canonical contract, and pass that adapter to the primitives; do not fork colors.
4. The matrix's dynamic tone function still maps meaningful `no`/`nothing` cells to `K.muteSoft`, even though the static audit does not enumerate each generated cell. Use `K.mute` for readable matrix text.
5. The success return uses `KTop action="more"`, while the canonical Costs ledger header uses the plus action. It also draws a new bespoke ledger row instead of consuming the existing `ExpenseRow` receipt/payer/split grammar. Return through `KTop title="Costs" action="plus"` and the canonical `ExpenseRow`, with any just-added emphasis applied as a non-owning wrapper.
6. The new row contains a private-use glyph before `RECEIPT`. Replace it with the established receipt icon treatment or plain `RECEIPT` text so rendering does not depend on an unsupported icon font.

### Status

M12 remains `DESIGNING`. The behavioral journey is now settled; only source integration, token correctness, and canonical ledger composition remain.

## Round 1 validation — twentieth design return (Receipt/OCR closed)

### Corrections that passed

- `ScanPrimary`, `ScanSecondary`, and `RevField` were removed as component implementations. Actions and editable review fields now consume canonical `VBtn` and `VField` directly; the remaining `revProps` helper supplies state props only and owns no visual geometry.
- A scoped `KP` adapter now maps existing Costs tokens into the shared primitive palette contract. It supplies the required field, border, semantic, and surface keys without inventing a parallel color system.
- `OR ENTER IT MANUALLY`, the corrected original amount, and generated negative/empty matrix values now render with readable meaningful-text tokens.
- The consistency audit returned from 38 findings to the established 36-finding project baseline. The baseline itself is not globally clean: it still reports one unindexed legacy HTML file, 36 unresolved meaningful-text findings, eight reviewed decorative findings, and 11 shared-primitive warnings. None is newly introduced by Receipt/OCR.
- The successful Add state now uses the canonical Costs header with the plus action and the canonical `ExpenseRow` receipt/payer/split grammar. The just-added emphasis is non-owning, and the unsupported private-use receipt glyph is gone.
- The settled journey remains intact: entrance, permission rationale and recovery, indeterminate upload, active and long-running scan behavior, complete/partial/unreadable results, manual correction, explicit confirmation, add-in-progress, retryable add failure, durable receipt/OCR truth, privacy, accessibility, and implementation-boundary notes.

### Status

**M12 Receipt/OCR scan states → DESIGN VALIDATED.** This closes all Round 1 design rows (M01–M12 and S01–S02). It does not establish production parity: scan fixtures, upload/OCR failure behavior, correction preservation, confirmation gating, expense creation and retry semantics, ledger return, protected receipt visibility, and accessibility behavior still require code-backed validation before M12 can become `PARITY VALIDATED`.
