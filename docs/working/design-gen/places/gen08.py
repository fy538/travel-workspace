...
def arrival(stairs=True):
    ...
    return I.access_compare([('BY FERRY', [(6, 'foot'), (20, 'wait'), (25, 'ride'), (9, 'foot')] + ([(2, 'stairs')] if stairs else []), 'PIER 11 · EVERY 40 · 42 MOVING, 20 DRAWN, 42 TO 82 IN ALL'),
                             ('BY THE B61', [(4, 'foot'), (6, 'wait'), (28, 'ride'), (3, 'foot')] + ([(2, 'stairs')] if stairs else []), 'EVERY 12 · 37 MOVING, 6 DRAWN, 37 TO 49 IN ALL · THE SAME STAIRS')], origin='FROM CANAL STREET', h=118)
...
def extension(part):
    ...
    if part == 'there':
        ...
        return phone(inner)
    if part == 'after':
        inner = anchor('THE HARBOR BOOK', 'SAT 3:40 PM', back=True, sub='Ch. 4 · the last page')
        inner += gut(f'<div>{serifline("The sill did what it was built to do, and the creek bed did what it had always done. Every high tide since has sorted the two out again, and the pier has gone under first each time.", 16, 22)}</div>', top=20)
        inner += gut(f'<div style="margin-top: 12px;">{serifline("What the engineers could not raise, they learned to drain; the park across the water is the other half of that lesson, and it is still running its pumps.", 16, 22)}</div>', top=0)
...
def rules_table():
    return tbl(['THE RULE', 'ENTITY 09 · 11 · 12', 'PLACES 08', 'SELECTED · PROPOSED'], [
        ['The shell', 'Photo, kind and town, name, faces, one context line; the same every time (09 A to C, the lab&rsquo;s older rule set)', 'Anchor, sentence, plate, people', 'Selected: Entity&rsquo;s invariant shell, drawn here as the anchor, one sentence, the plate, the people line and one context line'],
...
def board():
    ...
    row4 = [viewport(col(extension('before'), caption('08.10 · BEFORE · THE UNDERSTANDING', 'FRIDAY EVENING, FROM THE FIELD', 'Four minutes on why the pier floods first, and one sentence naming what will be visible tomorrow: two iron squares at the crossing'))),
            ...
            notecol('One connected extension (September 9, second pass)', [
                ...
                ('THE ENDING, SELECTED', N('The reading ends the way a reading ends: the chapter&rsquo;s last page and Back. There is no closing screen and no acknowledgment to give, because a ceremony at the end is still the app asking for a moment of attention. Nothing advances to lunch; nearby places remain on the place page under Near it, available when they are wanted.'))], w=1180)]
