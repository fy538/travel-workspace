...
def mistaken():
    ...
    inner += gut(relationship_trace('Because of the March afternoon: you asked for a level way in, so stairs are kept off what you are shown.', 'WHAT VESPER UNDERSTOOD &middot; ABOUT YOU'), top=16)
    inner += gut(door('That is not right') + door('The level afternoon', MUTE), top=16)
    return phone(inner)
def correction():
    """The correction, made legible: whose constraint it is, which outing it concerns, how long it lasts, and what it changes.
    Today still comes out level, because the father is here; the difference is that the rooms come back for a visit without him."""
    inner = anchor('THE PRINT ROOM', 'SAT 1:05 PM', back=True, sub='Red Hook · with your father, back by 5')
    inner += orientation('Understood: the stairs are his, and only while he is with you.', 'Today is still the level afternoon &mdash; for the reason you gave, not one about you', 26, 31)
    inner += gut(readback('YOU SAID', '&ldquo;That was for my parents.&rdquo;', GOLDD), top=16)
    inner += gut(hours_register([('WHOSE', 'Your father&rsquo;s, not yours'), ('WHICH DAYS', 'The March afternoon, and today, because he is with you'), ('AFTER TODAY', 'Nothing is kept; stairs come back on this page tomorrow')]), top=16)
    inner += gut(consequence('WHAT THIS CHANGES', 'Today reads the same and for the right reason: the level floor, the pier, the 3:20 boat back by 5. The rooms upstairs are on the page again, and the show runs to Sunday, so a Saturday on your own still reaches them.'), top=16)
    inner += gut('<div>' + prow('Rooms Remade, upstairs', 'TO SUNDAY &middot; A FLIGHT OF STAIRS &middot; NOT THIS AFTERNOON', first=True, last=True) + '</div>', top=16)
...
def later_visit():
    ...
    inner += orientation('Ink and Water, upstairs, from Saturday.', 'A print workshop on Van Brunt Street &middot; three weeks after the March afternoon', 26, 31)
    ...
    inner += gut(relationship_trace('A level afternoon here was planned in March, with your father.', 'NO VISIT RECORDED &middot; NOTHING KEPT ABOUT STAIRS'), top=16)
...
def board():
    ...
    r3 = [viewport(col(mistaken(), caption('03.7 · WHAT VESPER HAD UNDERSTOOD', 'A LIMITATION READ ONTO THE PERSON', 'Before the correction: one March afternoon with a father has become &ldquo;stairs are kept off your afternoons&rdquo;. The page says what it believes and where it got it, and offers the one door that matters'))),
          viewport(col(correction(), caption('03.8 · THE CORRECTION, MADE LEGIBLE', '&ldquo;THAT WAS FOR MY PARENTS&rdquo; · WHOSE, WHICH DAYS, HOW LONG', 'The three things the correction settles, said as rows: the constraint is his; it covers the March afternoon and today, because he is here; nothing is kept after today. Today still reads level, and the rooms return to the page for a Saturday alone'))),
          viewport(col(later_visit(), caption('03.9 · THREE WEEKS ON, ALONE', 'NOTHING PERSONAL WAS INHERITED', 'The stairs are a fact of the building again. The record says an afternoon was planned in March; it does not claim a visit happened, because nothing observed one'))),
