"""Vesper — Home · generous-value pass, revision 2: composition and production.
...
OUT = os.path.join(os.path.dirname(__file__), 'homeproj'); V1 = os.path.join(os.path.dirname(__file__), 'v1phones')
...
def ways_card(t, items, meta_t=''):
    ...
    for i, (a, b) in enumerate(items):
        inner += (f'<div style="display: flex; gap: 12px; align-items: flex-start; padding: 10px 0 {8 if i < len(items)-1 else 0}px 0; {"border-top: 1px solid rgba(27,23,20,0.07);" if i else "margin-top: 8px;"}">'
                  f'<span class="kick" style="min-width: 12px; color: {ANCHOR}; padding-top: 3px;">{i+1}</span><div style="flex: 1;">{title(a, 16, 21)}{sup(b)}</div></div>')
...
def rows_A(with_alex=True, with_dana=True, with_show=True, dentist='Dentist &middot; Tuesday 9:00 &middot; <span style="color: #6E6862;">24&deg; &middot; walk, the bus is slower</span>', alex_text=None):
    ...
    if with_dana: rows.append(arow('Dana &middot; Saturday the 19th &middot; <span style="color: #6E6862;">Sunday morning is hers &middot; she sent a bookshop</span>', avatars=['D']))
...
COLD = lambda when='Thursday night': u2(f'{when.capitalize()} the river side of your walk runs four degrees colder than the avenue',
    ...
    compare2('RIVER SIDE &middot; 9 PM', '24&deg;', 'wind off the water', 'THE AVENUE &middot; 9 PM', '28&deg;', 'masonry holds the day'), meta_t='FORECAST + STREET FORM &middot; FIXTURE')
...
def h1v2():
    ...
    inner += gut(ways_card('Today', [
        ('The loaf, then the water', 'Bakery by 10:30; low water on the pier at 1:40, nine minutes on.'),
        ('The flood line, walked at low water', 'Between 1:40 and 4 the granite kerbs show where the gates&rsquo; protection ends.'),
        ('Stay in. The skillet, preheated dry', 'Thursday&rsquo;s soggy crust was the pan, not the dough.'),
    ], 'LOW WATER 1:40&ndash;4 &middot; MARKET UNTIL 1'), top=16)
    inner += rows_A()
...
H1_NOTES = notecol('What changed, and what each unit earns', [
    ('TYPOGRAPHY, FOUR JOBS', N('Section headings are now sans, sentence case, with a hairline (&ldquo;In motion&rdquo;, &ldquo;Worth knowing&rdquo;, &ldquo;The city this week&rdquo;). Content titles are the serif line and the first thing in every unit. Metadata is one mono line, only for time, distance, date, or source. Supporting text is one or two sentences. The stacked gold kicker above every title is gone.')),
    ('CONTAINMENT, SELECTIVE', N('Three cards: Maya&rsquo;s share (media-led, authorship inside the unit), &ldquo;Today&rdquo; (a prepared afternoon with a readable shape), and the reading preview (wash, with the plate). Rows stay rows. The mechanism and the technique stay bare because they are findings, not objects. D-H1 (&ldquo;a crown only when operational&rdquo;) is withdrawn.')),
    ('TEXT REMOVED', N('&ldquo;Her words, kept hers&rdquo;, &ldquo;nothing to reply to&rdquo;, &ldquo;not inferred from you&rdquo;, &ldquo;try it or don&rsquo;t&rdquo;, and every footnote that explained why a unit qualified. Kept: authorship (name, when, audience), source metadata, and the fixture mark on the one bounded mechanism.')),
    ('LABELS REMOVED', N('&ldquo;Understanding now&rdquo; + &ldquo;Thursday&rsquo;s cold &middot; why your route splits&rdquo; + the title collapsed to one title. &ldquo;Something to try&rdquo; + &ldquo;Carried forward&rdquo; collapsed to the technique under &ldquo;Worth knowing&rdquo;. &ldquo;When you have four minutes&rdquo; became the reading card&rsquo;s own metadata.')),
    ('PAYOFFS RETAINED', ledger([('MAYA', 'Loaf, day, hour, distance, her photograph'), ('TODAY', 'Three distinct days, each with its reason'), ('IN MOTION', 'Alex&rsquo;s arrangement, Dana&rsquo;s handoff, the show, the dentist'), ('THE COLD', 'Two numbers and the mechanism'), ('THE SKILLET', 'A usable technique from the person&rsquo;s own note'), ('THE CITY', 'Two dated facts'), ('THE READING', 'The payoff sentence, then the door')])),
    ('DIAGNOSTICS', N('Word counts and scroll heights for the three phones are in the captions and on H0. They are diagnostics, not targets.')),
], w=440)

def H1():
    cols = [col(v1('study_sunday'), caption('ORIGINAL &middot; STUDY BOARD', 'The quiet Sunday', 'ONE READING, THE SEAM, TWO ROWS')),
            col(v1('h1v1'), caption('BEFORE &middot; H0&ndash;H5 PASS', 'Generous, text-heavy', 'STACKED KICKERS &middot; EVERY FAMILY A SECTION &middot; RATIONALE ON THE SURFACE')),
            col(h1v2(), caption('AFTER &middot; THIS PASS', 'Generous, composed', 'THREE CARDS &middot; FOUR TYPE JOBS &middot; RATIONALE REMOVED')),
            H1_NOTES]
    return board_page(1880, 3000, 'VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H1 &middot; ORDINARY SUNDAY &middot; PROPOSAL', 'H1 &middot; The generous Sunday, easier to scan',
...
def sunday_unit(inline=True):
    if inline:
        return u2('Sunday with Dana: the bookshop she sent opens at eleven; Maya&rsquo;s loaf is gone by then', 'Loaf first, six minutes to the shop, and the morning stays hers.', meta_t='DANA&rsquo;S LINK &middot; MAYA&rsquo;S SHARE &middot; NOTHING BOOKED')
    return u2('Sunday with Dana: the bookshop she sent opens at eleven', 'The morning stays hers, as she asked.', meta_t='DANA&rsquo;S LINK &middot; NOTHING BOOKED')
...
END_MON = lambda: ending(WEEK_MON, 'Friday the show. Saturday the market, then the pier. Sunday, Dana&rsquo;s morning.')
...
def h2v2_t2():
    inner = h2_read() + gut(alex_card(), top=22) + rows_A(with_alex=False)
    inner += sect('This week') + gut(market_card() + '<div style="height: 26px;"></div>' + sunday_unit(False))
    inner += sect('From your people') + gut(share_card('M', 'Maya', 'FRIDAY &middot; FRIENDS', 'The Sunset Park bakery does the sesame loaf on Sundays only. Go before eleven or it&rsquo;s gone.', 150, 'MAYA&rsquo;S PHOTOGRAPH &middot; SLOT', 'SIX MINUTES FROM THE BOOKSHOP DANA SENT', 'The bakery')
                                            + '<div style="height: 16px;"></div>' + handoff_card('D', 'Dana', 'THURSDAY &middot; TO YOU', 'This one, for Sunday? I want the poetry shelf in the back.', 'A USED BOOKSHOP ON COURT STREET &middot; OPENS 11', 'The bookshop'))
...
def h2v2_sparse():
    ...
    inner += sect('This week') + gut(market_card(False) + '<div style="height: 26px;"></div>' + sunday_unit(False))
    inner += sect('Worth knowing') + gut(COLD() + '<div style="height: 26px;"></div>' + SKILLET)
...
H2_NOTES = notecol('The comparison, revised', [
    ('WHAT CHANGED', N('Alex&rsquo;s Saturday is a contained arrangement card with its people and timing inside; the market is a card with its instrument and Theo&rsquo;s quoted judgment; Sunday is a bare unit. Treatment 2&rsquo;s region holds Maya&rsquo;s share as a media-led card and Dana&rsquo;s handoff as a compact card, once each. Footnotes about audience and reply debt are gone; authorship, audience, and &ldquo;Alex decides&rdquo; stay.')),
...
def H2():
    cols = [col(v1('h2v1_t2'), caption('BEFORE &middot; H0&ndash;H5 PASS', 'Treatment 2, text-heavy', 'FOR COMPARISON')),
            col(h2v2_t1(), caption('TREATMENT 1', 'Distributed', 'SOCIAL MATERIAL INSIDE THE UNITS IT CHANGES &middot; ONE PULL DOOR')),
            col(h2v2_t2(), caption('TREATMENT 2 &middot; RECOMMENDED', 'Distributed + a conditional grouping', '&ldquo;FROM YOUR PEOPLE&rdquo; ONLY WHEN &ge;2 AUTHORED UNITS ARE UNSPENT ABOVE')),
            ...
            H2_NOTES]
    return board_page(2960, 2690, 'VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H2 &middot; SOCIALLY ACTIVE WEEK &middot; PROPOSAL', 'H2 &middot; The present social world, two ways, composed',
                      'Identical evidence in both treatments: Alex&rsquo;s arrangement and his words, Theo&rsquo;s market judgment, Maya&rsquo;s bakery share, Dana&rsquo;s addressed bookshop, the show. Cards for the arrangement, the market, and the authored material; rows for commitments; one bare unit for Sunday.', cols, FOOT)

# ───────────────────────────── H3 v2 ─────────────────────────────
STUB = ticket_stub('The Hall', 'FRI &middot; SAMPLE', '8:00')
def sample_ticket():
    return sample_card('A MADE-UP TICKET, READ THE WAY YOURS WOULD BE', STUB, 'Leave from work, not home. The way back is the constraint.',
                       span('THE SHOW &middot; 8&ndash;10', 140, 250, '+25 HOME', start='5 PM', end='12', w=349),
                       ['Doors 8 &middot; arriving by 8:40 skips nothing &middot; coat check yes', 'After ten the surface route home adds 25 minutes'],
...
def h3v2_first():
    ...
    inner += sect('This week in the city', top=28) + gut(COLD() + '<div style="height: 18px;"></div><div>'
    ...
    inner += sect('What one thing turns into') + gut(sample_ticket())
    inner += gut('<div>' + row('A photograph of a dish &rarr; <span style="color: #6E6862;">the mechanism behind its texture</span>', mark='hollow')
                 + row('A friend&rsquo;s note &rarr; <span style="color: #6E6862;">a better window for the same place</span>', mark='hollow')
                 + row('Scattered tickets and photos &rarr; <span style="color: #6E6862;">one recoverable record</span>', mark='hollow', last=True) + '</div>', top=16)
...
def h3v2_sample():
    ...
    inner += crown(PLAN, 'SAMPLE &middot; FRIDAY &middot; DOORS 8:00', 'Leave for the show from work, not home', 'The way back is the constraint, not the way there.',
                   span('THE SHOW &middot; 8&ndash;10', 140, 250, '+25 HOME', start='5 PM', end='12', w=317), cta=None, fn='ARRIVING BY 8:40 SKIPS NOTHING &middot; COAT CHECK YES &middot; SURFACE ROUTE AFTER TEN +25').replace('margin: 22px 22px 0 22px', 'margin: 0 22px')
    inner += gut(callouts([('1', 'Hall, date, doors come from the ticket. Set times come from the hall when posted.'), ('2', 'The way there is timed from wherever you say you leave. Your location is not assumed.'), ('3', 'The way home is the part people forget.'), ('4', 'Reading a ticket books, saves, and shares nothing. If you bring yours, it is kept in Life with one-tap Undo.')]), top=22)
...
def h3v2_chat():
    ...
    inner += gut(f'<div style="max-width: 330px; display: flex; flex-direction: column; gap: 10px;">'
                 f'<div style="{SERIF} font-size: 17px; line-height: 24px; color: {INK};">Friday at the hall, doors 8. Leave from work, not home. Arriving by 8:40 skips nothing; coat check yes. After ten the surface route adds 25 minutes.</div>'
...
def h3v2_return():
    ...
    inner += crown(PLAN, 'FRIDAY &middot; YOUR TICKET', 'Leave for the show from work, not home', 'The way back is the constraint, not the way there.',
                   span('THE SHOW &middot; 8&ndash;10', 140, 250, '+25 HOME', start='5 PM', end='12', w=317), cta='Open Friday evening', fn='YOUR TICKET + TONIGHT&rsquo;S FORECAST &middot; NOTHING BOOKED')
    inner += gut(kept_row('Your ticket &middot; Friday &middot; in Life'), top=22)
    inner += sect('This week in the city') + gut('<div>' + fact('TONIGHT &middot; 18&deg;', 'The waterfront blocks freeze first; near the river it is four degrees colder than the avenue.')
...
def h3v2_next():
    ...
    inner += sect('Today in the city', top=28) + gut(u2('At low water the old creek mouth shows where the harbor used to come in', 'Walk the kerb line along the waterfront blocks between 1:40 and 4; the granite ends where the 1911 gates&rsquo; protection ends.', meta_t='TIDE TABLE + THE HARBOR BOOK, CH. 3 &middot; FIXTURE')
                                                     + '<div style="height: 18px;"></div><div>' + fact('THE GREENMARKET &middot; UNTIL 1', 'Bread thins out by ten; produce holds until noon. Nine minutes from the address you gave.', last=True) + '</div>')
    ...
    inner += sect('What one thing turns into') + gut(sample_card('A MADE-UP PHOTOGRAPH OF A DISH', f'<div style="width: 96px; height: 72px; border-radius: 10px; background: #2A241E; flex: none; position: relative;"><span style="position: absolute; left: 8px; top: 8px; width: 18px; height: 18px; border-radius: 9px; background: {GOLD}; color: {INK}; font-size: 10px; font-weight: 700; display: flex; align-items: center; justify-content: center;">1</span></div>',
                                                                   ...
                                                                   ['1 &middot; A thin film that holds the ridges &rarr; a starch-supported emulsion', 'How to test it tonight: sauce meets pasta ninety seconds early'], [('How it&rsquo;s read', GOLDD), ('Try with yours', INK)]))
...
H3_NOTES = notecol('The result, not the pitch', [
    ('WHAT CHANGED', N('The sample is now a ticket-shaped stub, an arrow, and the evening it becomes: the title, the span, two facts. The paragraph that described what a ticket could become is gone. The three further capabilities are rows, not numbered paragraphs. City material is one mechanism unit and two fact rows.')),
...
def H3():
    cols = [col(v1('h3v1_first'), caption('BEFORE &middot; H0&ndash;H5 PASS', 'First open, described', 'FOR COMPARISON')),
            col(h3v2_first(), caption('1 &middot; FIRST OPEN &middot; TUESDAY 8:05', 'Useful without history', 'THE CITY + THE SAMPLE RESULT')),
            ...
            H3_NOTES]
    return board_page(3260, 1815, 'VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H3 &middot; FIRST OPEN &rarr; NEXT OPEN &middot; PROPOSAL', 'H3 &middot; Show the transformation; do not describe it',
...
# ───────────────────────────── H4 v2 ─────────────────────────────
SHOW_CROWN = lambda fn: crown(PLAN, 'FRIDAY &middot; DOORS 8:00', 'Leave for the show from work, not home', 'The way back is the constraint, not the way there.',
...
def h4v2_before():
    ...
    inner += ending(WEEK_THU('Alex'), 'Tomorrow the show. Saturday the market, then Alex. Sunday, Dana&rsquo;s morning.')
    return phone(inner, 0)
...
def h4v2_after():
    ...
    inner += sect('Worth knowing') + gut(COLD('tonight') + '<div style="height: 26px;"></div>' + SKILLET)
    inner += sect('The city this week') + gut(CITY_OH)
    inner += ending(WEEK_THU('the park'), 'Tomorrow the show. Saturday the park at 6:30. Sunday, Dana&rsquo;s morning.')
    return phone(inner, 0)

H4_NOTES = notecol('The same rules, under priority', [
...
def H4():
    cols = [col(h4v2_before(), caption('BEFORE &middot; THURSDAY 5:40', 'The fuller Thursday', 'CROWN + ROWS + WORTH KNOWING + THE CITY')),
            ...
            H4_NOTES]
    return board_page(1880, 1825, 'VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H4 &middot; PRIORITY STRESS TEST &middot; PROPOSAL', 'H4 &middot; A genuine priority, checked against the revised system',
...
def h5v2():
    ...
    inner += gut(ways_card('This afternoon', [
        ('The pier at low water, 2:40&ndash;5', 'The first walk back can be the one you know. The market tomorrow until 1.'),
        ('The Sorrento dish, for Saturday&rsquo;s three', 'The texture you photographed is mostly timing. A dish for three is the honest test.'),
        ('Stay in', 'The noodle shop delivers until ten.'),
    ], 'LOW WATER 2:40&ndash;5 &middot; PHOTOS IMPORTING, 412 OF 690'), top=22)
    ...
    inner += sect('Worth knowing') + gut(u2('The last minute at the stove explains the texture better than the ingredient list',
        'The sauce clings as a thin film and the finish looks integrated at service &mdash; consistent with a starch-supported emulsion finished in the pan. It does not prove how that kitchen made it.',
        ...
        meta_t='YOUR PHOTOGRAPH &middot; SORRENTO, LAST TUESDAY', door_text='Test it Saturday, for three'))
    inner += sect('From the trip') + gut(u2('Sorrento &rarr; Amalfi is the only leg documented from departure to arrival',
        'Ferry ticket 9:10, the harbor photograph 9:14, the Amalfi quay 10:02. Every other leg has one end missing.',
        ...
        meta_t='FROM TICKETS + 412 PHOTOGRAPHS SO FAR &middot; FILLS IN AS IMPORT FINISHES', door_text='The trip, in Life'))
...
def H5():
    cols = [col(v1('day0_current'), caption('ORIGINAL &middot; STUDY BOARD', 'Day zero as delivered', 'AN IMPORT CARD LEADS; &ldquo;UNPACK&rdquo; CLOSES')),
            col(v1('h5v1'), caption('BEFORE &middot; H0&ndash;H5 PASS', 'New York first, text-heavy', 'FOR COMPARISON')),
            col(h5v2(), caption('AFTER &middot; THIS PASS', 'New York first, composed', 'ONE CARD &middot; ROWS &middot; TWO FINDINGS &middot; ONE FACT')),
            H5_NOTES]
    return board_page(1880, 2230, 'VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H5 &middot; RETURN HOME &middot; PROPOSAL', 'H5 &middot; Coming home, checked against the revised system',
...
def H0(diag):
    ...
    diag_t = blk('BEFORE / AFTER DIAGNOSTICS &middot; VISIBLE WORDS AND SCROLL HEIGHT (NOT TARGETS)', tbl(['PHONE', 'BEFORE (H0&ndash;H5 PASS)', 'AFTER (THIS PASS)', 'REMOVED', 'RESTORED / RETAINED'], [
        ['H1 Sunday', diag['h1v1'], diag['h1v2'], 'Gold kicker above every title; 7 footnotes; &ldquo;Understanding now&rdquo;, &ldquo;Something to try&rdquo;, &ldquo;When you have four minutes&rdquo; as headings', 'Cards: Maya&rsquo;s share, Today, the reading. All seven payoffs'],
        ['H2 Treatment 2', diag['h2v1_t2'], diag['h2v2_t2'], 'Kickers; audience footnotes; &ldquo;her words, kept hers&rdquo;; the region&rsquo;s explanatory footer', 'Cards: the arrangement, the market, the share, the handoff. Sunday as one bare unit'],
        ['H3 first open', diag['h3v1_first'], diag['h3v2_first'], 'Three paragraph-length city units &rarr; one mechanism + two fact rows; the sample&rsquo;s description &rarr; the result; three numbered paragraphs &rarr; three rows', 'The stamped sample; the three capabilities; &ldquo;Bring one thing&rdquo;'],
    ]))
    prod = blk('PRODUCTION LEDGER &middot; EVERY VISIBLE UNIT, BY CLASS', tbl(['UNIT', 'PRODUCTION CLASS', 'LIKELY TRIGGER', 'REUSE / AUDIENCE SCOPE', 'FRESHNESS / INVALIDATION', 'FALLBACK', 'COST UNKNOWN'], [
        ...
        ['Arrangement card (Alex)', 'Structured state (Occasion) + owner&rsquo;s words', 'Occasion changes', 'Participants only', 'Any Occasion write; expiry', 'Row only', 'None new'],
        ...
        ['Handoff (Dana)', 'Human-authored + Place', 'Addressed material', 'Recipient only', 'Withdrawal; Place change', 'Row only', 'None new'],
        ['Theo&rsquo;s note inside the market', 'Human-authored (may-use)', 'Grant covers the market', 'Recipient only', 'Grant expiry reverts the window', 'Rhythm without the quote', 'None new'],
        ...
        ['The cold split (mechanism)', 'Reusable researched content + world information', 'A forecast condition matches a stored mechanism', 'Shared explanation; personal numbers', 'Forecast change re-composes numbers, not prose', 'Forecast line only', 'Authoring the mechanism library'],
        ['The skillet technique', 'Reusable researched content, selected by the person&rsquo;s note', 'The note exists; not yet tried', 'Shared technique; private trigger', 'Retires after tried or ignored twice', 'Not shown', 'None new'],
        ...
        ['Sample card + sheet', 'Fixed, labelled demonstration', 'No personal history for that capability', 'Shared across accounts', 'Retires after tried or ignored twice', 'The city carries the page', 'None: prepared once'],
        ['The dish photograph finding', 'Personal relevance: reusable mechanism + two personal callouts', 'A photograph with a matching mechanism', 'Private', 'Correction invalidates callouts', 'Photo without callouts, or not shown', 'Vision pass per photo'],
        ...
        ['Priority crown with two-place comparison (H4)', 'Structured state (Occasion candidates)', 'Owner asks a decision by deadline', 'Participants', 'Deadline; owner settles', 'Rows only', 'None new'],
    ...
    handback = blk('HANDBACK', N('<b>Boards (project &ldquo;Vesper &mdash; Home&rdquo;), revised in place with the previous pass kept beside them:</b> H0 - Ledger and Decisions &middot; H1 - Ordinary Sunday Generous &middot; H2 - Social Week Two Treatments &middot; H3 - First Open to Next Open &middot; H4 - Priority Stress Test &middot; H5 - Return Home Corrected. Board 1&ndash;4 untouched.<br><br>'
    ...
    inner = (head('VESPER &middot; HOME &middot; GENEROUS-VALUE PASS &middot; REVISION 2 &middot; H0 &middot; COMPOSITION, PRODUCTION, DECISIONS, HANDBACK', 'H0 &middot; One composition, its production ledger, and what needs a ruling',
                  'The revised system; before/after diagnostics for H1&ndash;H3; every visible unit classified by production class with trigger, reuse, freshness, and fallback; the decision log with D-H1 withdrawn and D-H10/D-H11 added; the fifteen questions per page; the handback.')
             + '<div style="display: flex; flex-direction: column; gap: 34px;">' + system + diag_t + prod + ops + decisions + review + handback + '</div>')
    return (HEAD + f'<div style="width: 1560px; min-height: 3025px; background: #F4F0E7; box-sizing: border-box; padding: 30px 32px 36px 32px; {SANS} color: {INK}; display: flex; flex-direction: column;">' + inner + f'<div class="fn" style="margin-top: 30px; line-height: 16px;">{FOOT}</div></div>' + TAIL)

PHONES_FOR_DIAG = {'h1v2': h1v2, 'h2v2_t2': h2v2_t2, 'h3v2_first': h3v2_first}

if __name__ == '__main__':
    import json
    diag = json.load(open(os.path.join(OUT, 'diag.json'))) if os.path.exists(os.path.join(OUT, 'diag.json')) else {k: '&mdash;' for k in ['h1v1','h1v2','h2v1_t2','h2v2_t2','h3v1_first','h3v2_first']}
