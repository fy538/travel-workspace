...
expires: 2026-09-09
why_new: Records a time-bounded naming exploration and collision screen without changing product or release authority.
source_of_truth_for:
  - brand-naming-exploration-2026-08
---

# Brand naming: recognizable spelling variants and Greek-root plays
...
The strongest new naming mechanism is not an opaque invented word. It is a
familiar, conceptually useful word with one restrained spelling change:

- **Beckon -> Bekon**
- **Setting -> Settin**

The reader should be able to recover the source word, pronunciation, and core
idea immediately. The alteration exists to create a compact visual identity and
practical naming space; it should not make the name look encrypted, misspelled,
or generically startup-like.

This direction is closer to the desired Aesop / Sonos / Notion / Oura / Figma
standard than multi-syllable synthetic names. Meaning comes first, but current
availability is an entrance criterion rather than a cleanup step.

## Transformation rules

A variant should:

1. preserve the source word's dominant sound;
2. change only one obvious feature: a final letter, a familiar phonetic swap,
   or one nonessential vowel;
3. remain easy to say after seeing it once;
4. remain easy to spell after hearing it once;
5. avoid a visually broken consonant pile;
6. survive exact-name searches in consumer software, AI, maps, local discovery,
   planning, travel, social coordination, and proactive assistants;
7. preferably have the exact `.ai` available. An open exact `.com` is valuable,
   but requiring both domains should not force a bad name.

Rejected transformation examples include `Setng`, `Amydst`, and `Situatn`.
They create availability by destroying fluency.

## English-root results

### Current leaders

| Name | Source | Domain screen | Strategic read | Current decision |
|---|---|---|---|---|
| **Settin** | Setting | `settin.ai` open; `.com` registered | Place, context, atmosphere, circumstance, and narrative setting | **Advance** |
| **Stirin** | Stirring / Astir | `stirin.ai` open; `.com` parked | Something worthwhile is stirring nearby; proactive, ambient, and alive | **Advance** |
| **Elswher** | Elsewhere | exact `.com` and `.ai` open | Discovery and possibility beyond the obvious | **Advance with travel-skew caution** |
| **Amungst** | Amongst | exact `.com` and `.ai` open | Being among people and places; strong multiplayer frame | **Language-test** |
| **Tydin** | Tidings | `tydin.ai` open; `.com` registered | Timely intelligence arriving from the world | **Language-test** |
| **Amids** | Amidst | `amids.ai` open; `.com` registered | Presence within a place or unfolding situation | **Hold** |
| **Somewhre** | Somewhere | `somewhre.ai` open; `.com` registered | Place, mystery, and possibility | **Hold; visually less clean** |
| **Noticin** | Noticing | `noticin.ai` open; `.com` registered | The system notices changes and opportunities for the user | **Hold; behavior rather than world** |
| **Unfoldin** | Unfolding | `unfoldin.ai` open; `.com` registered | The world continuously unfolding around the user | **Hold; adjacent to Unfold** |

The clearest distinction between the two leaders is:

- **Settin** names the contextual world the product understands.
- **Stirin** names the changing real-world intelligence the product delivers.

### Collision-led rejections

| Name | Reason to stop |
|---|---|
| **Bekon** | Multiple active uses, including a location-based social app, a social-dining app, and an enterprise AI company. The sound-equivalent family is too crowded. |
| **Happenin** | An active local-life product already combines events, nearby discovery, group planning, booking, availability, and notifications. |
| **Gatherin** | Existing group-activity and social apps use the exact name. |
| **Nerby** | An active local community and discovery app uses the exact name. |
| **Astyr** | Active US word-mark registration. |
| **Amyst** | Existing low-code software and development-company uses. |
| **Mongst** | Active US word-mark registration. |
| **Rootin** | Multiple active habit and mobility apps. |
| **Setten** | Active Belgian design, AI, and technology agency. |

## Greek-root plays

### Method

Greek should be treated as a source of compact concepts, not as an aesthetic
costume. Raw transliterations such as `Metaxy` or `Aletheia` often fail the
speech test. The useful pattern is:

1. start with a Greek idea that genuinely describes the product;
2. create at most one phonetic or orthographic adjustment;
3. make the English pronunciation recoverable;
4. screen the adjusted spelling before becoming attached to the etymology.

### Most relevant roots and variants
...
| **Estya** | `Hestia` / Ἑστία | Hearth and the center of domestic life | `estya.ai` open; `.com` registered | Warm and recurring-place oriented; substantial existing Hestia/Estia noise |
...
| Name | Reason to stop |
|---|---|
| **Dipla** | Although δίπλα means “next to” or “beside,” an active local-community app already uses Dipla for nearby people, places, activities, and real-time local posts. This is a direct product collision. |
| **Nostia** | Direct collision with an active social travel and group-planning app, plus an AI journal and current software/location trademark activity. |
| **Steki** | The meaning is nearly perfect—a habitual place or hangout—but the raw word is crowded by hospitality uses and adjacent software called Stekio. |
...
These are exploration leaders, not adoption recommendations:
...
3. ask the participant to spell it after hearing it aloud;
4. test ten-minute unaided recall;
5. ask what sort of company it sounds like;
6. show it as an app icon, notification sender, spoken referral, and investor
   sentence;
7. only afterward reveal the source idea and record whether that meaning makes
   the name better or merely rationalizes it.

Advance no more than three names to professional US/EU/UK/China knockout
searches and native-speaker review.

## Research references

- Greek `steki` as a haunt, spot, or frequently visited hangout:
  https://www.wordreference.com/gren/%CF%83%CF%84%CE%BA%CE%B9
- Greek `dipla`, “next to” or “beside”:
  https://en.wiktionary.org/wiki/%CE%B4%CE%AF%CF%80%CE%BB%CE%B1
- Greek `nostos`, return home or homecoming:
...
- It's Happenin local discovery app:
  https://play.google.com/store/apps/details?id=com.itshappenin
- GatherIn group-activity app:
  https://apps.apple.com/no/app/gatherin-gather-with-friends/id6737059504
- Nerby local community app:
  https://play.google.com/store/apps/details?id=com.mwn.nerby
- Dipla local community app:
