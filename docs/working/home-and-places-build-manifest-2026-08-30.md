...
| `people_authored_region` | With people | banded region of cards (`Addressed to you`) | renders only when ≥2 contributions **addressed to this person** are not already spent in full above; never duplicates; disappears when empty; ends with the pull door to Places · From friends. Casual shares live in Places (decision §2, the social split) | **BUILD** — grant-scoped share reads exist (MP1); the region composer is new |
