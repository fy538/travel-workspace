...
functional slices described above. Backend
...
Backend verification-only successor `5e6f1d174` adds the R02 viewer/release
matrix without changing product behavior or the API. Its focused discovery and
exact-read packet passed **28 tests**; it uses a fake connection, so persisted
active/revoked release-ledger acceptance remains outstanding (see the review
ledger).
...
recorded as 55 expired API-policy reviews, 59 expired schema-bridge exceptions,
and 12 query-key ownership
...
wave):** for all package decisions below, use workspace `7eda5d2`, backend
`cc357a7c7`, and app `aa7167722`. The backend root-projection packet now passes
**482 tests**; the full offline backend gate now passes **21,661 tests** with
14 skipped and 53 expected passes, and the app packet passes **566 tests across
55 suites** with TypeScript passing. The latest changes repair semantic-owner
selection expectations, make shared Outcome shadow writes use one authoritative
Life lens while retaining lens membership, and record a bounded compatibility
window instead of leaving expired registrations. Earlier tuples in this table
remain historical receipts for their named slices. They do not establish
recurring supply, native media/device acceptance, visual parity with an
external Claude export, or release readiness.
The active primary build remains a longer, benefit-led Home composition from
...
**September 22 continuation reachability repair:** backend commit `8c85a1d83`
closes the bounded compiler gap described above. A governed
`CompositionBriefV1` continuation now becomes a reachable root action only for
the two capabilities the existing native surfaces already understand:
`source.inspect` (with duplicate attachment inspection removed) and
`chat.continue` (with a typed Chat destination carrying only existing seedable
owner refs). `ValueContract.actionable` now reflects emitted actions rather
than the presence of arbitrary metadata. SAVE, ACT and unknown capabilities
remain non-actionable until their owner, confirmation and consequence
contracts are explicit; no generic capability router was added. The full
backend `tests/root_projection` packet passes **476 tests**, Ruff/format checks
and backend commit hooks pass, and no API schema, generated type, route or app
file changed. This closes compiler-to-native reachability for the supported
cases, not the still-open automatic recent-return supply trigger.

## September 22 functional implementation receipt — separate Home original seats

The Home original-delivery adapter previously gave every recipient-authorized
delivery the same `home.now.original-delivery` surface seat. The generic
admission layer therefore suppressed all but the first delivery, despite the
adapter's bounded limit of two and despite each delivery carrying a distinct
Relationships grant, revision and custody boundary.

Backend commit `6e1e4aede` makes the seat identity delivery-specific
(`home.horizons.original-delivery.<delivery_id>`). It changes no audience,
custody, MIME, source, or retention policy, and it does not merge separate
shares into an authored photo set. The focused regression proves two distinct
active originals survive Home selection with their exact delivery references;
the broader Home/source/contract packet passes **145 tests**. Ruff and format
checks pass, and backend commit hooks pass. This closes a bounded receiving
and composition defect; native Home original delivery remains the previously
recorded one-recipient text acceptance, while binary-media/device acceptance
and broader social composition remain open.

## September 22 functional implementation receipt — exact Trip bridge return

The legacy Plan bridge's live and recent-return Home units already carried an
exact `trip` resource while using the existing `life.open` capability. The
mobile typed-destination resolver previously treated `trip` as neither an
exact Life record nor a recognized destination, so those units fell through to
the generic Life root and lost the specific Plan context.

App commit `bf9d764ac` now recognizes a non-empty Trip ref on a typed Life
destination and opens the canonical Trip detail route with the existing Home
return token. It does not change the owner model or migrate Plan storage; it
preserves the current legacy Trip owner until that migration is explicitly
accepted. The navigation suite passes **54 tests**, TypeScript passes, and
targeted ESLint reports no errors. This closes exact destination/return for
the supported Trip bridge; it does not establish native Home visual parity or
replace the remaining Plan/Trip migration work.

## September 22 functional implementation receipt — exact upcoming Places scope

Upcoming Trip Home units already carried an exact Trip resource through the
`places.open_context` capability, but the mobile resolver only recognized an
explicit `places_context` handle. The resulting route opened an unscoped
Places root and discarded the Plan context.

App commit `d33dd65f2` recognizes a Trip ref on that typed Places destination
and opens the existing Home-depth Places route with `tripId` plus the exact
Home return token. It does not add a new context object or infer a location.
The navigation suite passes **55 tests**, TypeScript passes, and targeted
ESLint reports no errors. This closes the supported upcoming-Plan → Places
scope/return seam; it does not claim current-world Places supply or visual
parity.

## September 22 functional implementation receipt — scoped social fallback

Some Places social cards carry a person and the current Places context but no
exact venue IDs. The backend intentionally preserves that bounded social
perspective rather than guessing a venue; the mobile resolver previously
dropped both refs and opened an unscoped Places root.

App commit `350970728` now routes that typed person-plus-context fallback to
the existing Friends section, retaining Home's return token. It does not infer
attendance, create a profile-to-venue edge, or alter the relationship owner's
audience decision. The navigation suite passes **58 tests**, TypeScript passes,
targeted ESLint reports no errors, and app hooks pass. This closes a social
fallback destination seam; exact venue doors remain the preferred path.

## September 22 functional implementation receipt — exact Home Plan proposal card

Open Home Plan-proposal units already carried one exact `plan_proposal` ref and
its owning `trip`, but the mobile typed-destination resolver treated the
`chat.open_plan_proposal` capability like a generic Chat continuation. That
discarded the proposal identity and could open a fresh trip-scoped conversation
instead of the canonical proposal card.

App commit `e32499922` recognizes the exact proposal/trip pair and routes to
the existing `tripChatProposal` entry with the Home return token. It fails
closed for an ambiguous proposal set and keeps the existing trip-scoped Chat
fallback without pretending to select one decision. The navigation suite passes
**57 tests**, TypeScript passes, targeted ESLint reports no errors, and app
hooks pass. This closes proposal destination/return reachability for the
supported Home unit; it does not change proposal authority, resolution, or
establish native visual parity.

## September 22 functional implementation receipt — Life refinding itinerary-block route

Life's deterministic refinding endpoint already returns exact owner paths for
itinerary blocks (`/trips/{trip}/plan/blocks/{block}`). The mobile destination
resolver did not recognize that owner path, so a valid refinding result could
render as display-only even though the existing Plan reader could open the same
block. App commits `7d6c033e1` and `5f22ea977` translate only that strict
two-segment path to
the canonical `routes.planBlock` route, preserves a root return token through
the shared resource helper, and leaves malformed paths unsupported. The
`LifeRefindLane` consumer now has a regression proving the exact row opens the
Plan reader.

The focused resource/navigation/refind packet passes **75 tests**; TypeScript,
targeted ESLint and diff checks pass. The later identity guard requires the
block segment in the owner path to match the `ResourceRef` id before opening
Plan. This closes refinding selection → exact Plan owner reachability; it does
not claim native refinding acceptance, broader Plan migration, or visual parity.

## September 22 functional implementation receipt — received-original ordering

The focused Home run found that two independently authorized original
deliveries were admitted into separate seats but could be reordered by their
random UUIDs when their declared priority was equal. The Home adapter now
preserves the relationship owner’s supplied order (currently newest first)
through a bounded, index-derived priority. It changes no recipient grant,
custody, MIME, expiry, or source policy and does not merge the deliveries.

The targeted Home portfolio packet passes **68 tests**; the combined root /
Life packet passes **517 tests**. Ruff, formatting, diff checks, and backend
pre-commit hooks pass. This closes deterministic ordering for the existing
received-original slice; it does not claim broader Home composition, native
visual parity, or release readiness.

## September 22 functional implementation receipt — owned non-Trip action receipts

The Home action-receipt owner now also supplies the viewer's own recent
non-Trip receipts. The reader filters to the exact `user_id` and
`trip_id IS NULL`, while the existing Trip-membership read remains responsible
for group/public receipts from other participants. Home merges the two reads
by receipt identity before applying its existing bounded continuity candidate,
so a supported consequence outside a Trip can reach Home without becoming a
social receipt feed or changing the receipt owner.

Backend commit `3f677bc97` adds the viewer-scoped query and Home adapter path.
The focused action-receipt/Home portfolio packet passes **91 tests**; the
canonical receipt/compiler packet passes **69 tests**; Ruff, formatting, diff
...
runs cover the exact current backend/app heads (`2e123173c` / `7a0af5f80`)
...
## September 22 mobile receipt-destination regression

App commit `7a0af5f80` adds the missing direct navigation regression for the
latest Home receipt destination. An owned `action_receipt` whose target is a
routable `venue` now has an explicit test proving that the canonical
`/venue/[id]` route opens and the Home root-return token survives. The focused
`rootProjectionNavigation` suite passes **60 tests** and app TypeScript passes.
...
The existing Life refinding contract already returns structured governed
answers for occurrence questions, plan listings, skipped sets, and identity
questions. The mobile lane previously dropped `governed_answer` and showed only
supporting result rows. App commit `3faaa2f18` now renders a compact direct
answer block before those rows, using only the typed answer kind, structured
detail, and existing result identities. It keeps “No evidence either way”
distinct from “You didn't go” and preserves each row's exact owner destination.

The focused Life presentation/component packet passes **19 tests**; the wider
Home/Life navigation packet passes **133 tests**; TypeScript, targeted ESLint,
and diff checks pass. This closes answer delivery for the existing refinding
contract; it does not add persistence, inference, repair mutation, or native
visual/device acceptance.

## September 22 functional implementation receipt — Places experience collection reflow

The Places render plan now keeps the authored experience-collection identity
separate from its horizontal-rail treatment. When large type forces that rail
into a vertical list, the renderer preserves the collection's compact copy
(category without a repeated place label or long description) and left-aligns
the content and action geometry for readable wrapping. Ordinary experience
cards and the horizontal rail retain their existing treatment. No backend,
schema, route, or producer behavior changed.

The focused Places packet passes **88 tests** across the render plan, section
feed, core helpers, and workspace kit; TypeScript, targeted ESLint, and diff
...
The Home original-delivery path currently supports an exact
recipient-authorized original read with sender attribution, expiry/withdrawal
revalidation, and Home return. Relationships does not yet expose a typed human
Reply, private Ask, or sender-chat destination to this root. Do not add a
...
Backend commit `756f9cc8a` carries forward the existing Home attention-breadth
repair and adds a compiler-level acceptance for its selected long scroll. After
the dominant unit, admitted candidates from a new region are considered before
additional repeats from a region already represented; compilation now has a
regression proving that the crown, four region seats, stable unit identities
and exact source refs survive together. No feed, generator, store, semantic
kind or social route was added. The focused Home composition suite passes
**11 tests** and the full backend `tests/root_projection` packet passes
**481 tests**. This closes a composition-contract evidence gap only; recurring
provider supply, native/device evidence and external design parity remain open.

## September 22 verification repair — editorial containment

App commit `2eefb9c1d` corrects one stale Home renderer assertion. The runtime
rule adopted by `5bebc401c` keeps editorial readings
(`horizon_editorial_passage`) uncarded while prepared possibilities and
prepared alternatives remain contained objects; the test had continued to
expect every composition to be a card. The focused Home/Places app packet
passes **57 tests**, and TypeScript passes. This changes no runtime behavior or
product contract; it restores verification alignment with the current visual
rule. Native visual parity and the broader Home composition milestone remain
open.
...
tuple is workspace `7eda5d2`, backend `cc357a7c7`, and app `aa7167722` on the
isolated functional-implementation branch. The code is materially more
...
1. **Trace the registered Home design.** For each substantial section, record
   its real producer, candidate/read adapter, selected region, renderer,
   destination/action and return behavior. Classify it as supplied-but-omitted,
   supplied-but-repetitive, or genuinely unsupplied. Do not treat the newer
   exploratory `Downloads/vesper-home` export as an implementation contract
   until the registered design authority is explicitly changed and verified.
2. **Compose from existing owners.** Build the longer benefit-led Home by
   improving ordering, breadth, hierarchy and copy around material the current
   owners already supply. Extend an owner only when the trace proves a valuable
   outcome with an existing authority, privacy, freshness, lifecycle and return
   contract. Do not add a generator, feed, store, semantic kind or social route
   solely to make the scroll look full.
3. **Accept one complete composition.** Exercise two materially different Home
   postures on the current tuple, follow one selected item through exact source,
   depth/action and return, and verify sparse, pending, failure, accessibility
   and native hierarchy behavior. Mock data may prove layout; it cannot close
   owner-read, device, visual-canon or release claims.

### Parallel but bounded closeouts

The selected addressed-note withdrawal/expiry path, fixed Home sample
presentation/retirement, ordinary empty states, binary-media runtime and the
named R02/R05/R06/R09 evidence can proceed in their owners' lanes. They do not
block ordinary Home composition and must not become a new architecture or
verification program. The recent-return trigger remains a separate policy
decision; no automatic cross-time adapter should be implemented until its
source, purpose, audience, lineage, correction/withdrawal, expiry,
duplicate/no-op and sparse behavior are accepted.
...
destination, and return without stale or invented context. The existing
482-root / 21,661-full-backend and 566-app test packets are supporting evidence,
not the exit itself.

## September 22 Home continuity reconstruction receipt

Backend `cc357a7c7` closes one concrete composition gap without adding a store,
generator, route family, or new owner. When the viewer-relative Experience
Graph already contains at least two Outcomes linked to the same explicit
Occasion, Home now composes those exact records into the existing
`continuity_reconstruction` sequence kind. Each step preserves its Outcome
state, bounded meaning and source identity; the unit carries an exact Life door
to the latest Outcome and a `life.read` re-read over all sequence sources. A
single Outcome, an Outcome without an explicit Occasion, and an unrecognized
association retain the existing individual continuity behavior.

App `aa7167722` promotes that already-defined semantic kind through the native
Home renderer registry and the existing composition renderer. No generic
fallback card was added. Focused backend Home/contract tests pass **108** and
the full `tests/root_projection` packet passes **482**; the focused Home
renderer/root-experience packet passes **30**, with app TypeScript and contract
typecheck, backend Ruff/format and hooks passing. The earlier full offline
backend gate (**21,661 passed, 14 skipped, 53 xpassed**) and broad mobile packet
(**566 tests / 55 suites**) remain valid evidence for their prior tuples, not a
fresh full-gate claim for this slice.

This is a real Home receiving/composition increment, not proof of the complete
longer Home milestone. Native device acceptance, verified external design
