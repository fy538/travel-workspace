...
not fresh verification of the current code tuple. Before this documentation
update, the isolated tuple was workspace `67ae496`, backend `2d0721e67`, and app
`93a9090de`; the latest backend fixture-isolation repair and app Life audio
reader do not add Home composition or current-tuple Home visual acceptance.
They therefore do not change the next package: trace and build a substantially
richer Home from material its owners actually supply, then accept that
composition against a verified design reference and representative postures.
