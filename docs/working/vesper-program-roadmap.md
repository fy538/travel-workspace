...
| --- | --- | --- |
| **Primary build — Home design-aligned composition breadth** | The current product-code tuple is workspace checkpoint `f8f90e8`, backend `3f677bc97`, app `1cca51a04`. Home has ten bounded owner readers, a prepared private Source-contribution read, posture/region selection and 18 promoted semantic renderers. It receives exact saved-Place change notices, a conditional Home → Places Friends door, a bounded current public Place reading in the explicit-save seat, an accepted Outcome → Places door when the Outcome already has an explicit routable Place binding, and the viewer's own bounded non-Trip action receipts merged with the existing Trip-membership receipt read. Source-contribution continuations now reach native `source.inspect` and `chat.continue` actions in the supported cases; SAVE/ACT remain deliberately non-actionable. The saved-Place HTTP/Postgres receipts remain at backend `4357d1320`; the later Outcome continuation is covered by **113** focused Home portfolio/page-composition/contract tests at `b3d5cf1c5`, the continuation repair by the full **476-test** root-projection packet at `8c85a1d83`, and the separate-original-seat correction by the **145-test** Home/source/contract packet at `6e1e4aede`. The legacy Plan bridge now preserves exact Trip detail navigation and Home return, upcoming Plan doors preserve the Trip scope inside Places, open Home Plan proposals focus the canonical proposal card in Chat, and person-only social handoffs preserve the Places context and Friends section through app `350970728`; Life refinding now opens exact itinerary-block owners and surfaces the governed direct answer before supporting rows through app `3faaa2f18` (19 focused presentation/component tests, 133 wider Home/Life navigation tests, TypeScript and targeted ESLint passing). The latest Places experience-collection reflow preserves compact copy and left-aligned action geometry at large type through app `1cca51a04` (88 focused Places tests, TypeScript and targeted ESLint passing). Earlier evidence includes a three-family owner-backed Home scroll, exact Outcome/Place return paths, and the secondary-composition renderer correction. The audio-reader app change does not alter Home. These receipts establish connected slices, not a substantial, design-aligned Home composition, recurring supply, or visual parity; no external canonical Home bundle is registered in this lane. | **Build sequence:** **(1) Trace** substantial sections in the registered Home design authority (`docs/governance/home-surfaces-design-authority.json`) against real producers, candidate/read, selected region, renderer, destination/action and return. Verify the external canonical bundle and its hashes before visual acceptance; exploratory exports are not runtime contracts. The latest `vesper-home` proposal includes shared-photo-set and event concepts that current owners do not supply; do not infer shared-set identity from separate recipient grants or recast saved-venue notices as city events. Classify each remaining gap as already supplied but omitted, supplied but weak/repetitive in composition, or genuinely unsupplied; record owner and evidence for the last category. **(2) Compose** a coherent longer Home from supported material first, making importance, variety and hierarchy legible without turning it into an infinite feed or adding user homework. Extend a producer only where the trace proves a valuable missing outcome and its owner, evidence, privacy, freshness, lifecycle and return are already defined. No new generator/store, semantic kind or isolated card/route to simulate fullness. **(3) Accept** two materially different Home postures against a verified design reference and current code tuple: follow at least one exact source/depth/action destination and return; inspect whole-scroll hierarchy, sparse/pending/failure states, accessibility and polished native treatment. Do not count a fixture-only scroll, isolated card or transport seam as the outcome. **Closeout, not a competing milestone:** fixed Home-sample native presentation/retirement, selected-note withdrawal/expiry, ordinary empty states, binary-media runtime, comprehensive visual parity and standard landing gates remain separate. The registered Places mock `places-search-loading` assertion is a separate QA gap; its forced-state failure produced no screenshot. |
