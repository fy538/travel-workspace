...
| **Primary build — Home/Places useful breadth** | In progress. The current candidate has real Home/Places owner reads, social receiving, practical fit/open-now, exact Place destinations and return; the same-Place supplied sequence now survives the canonical Home composition service with an honest source-unavailable omission. The remaining gap is disposable-Postgres/HTTP/device readback plus enough substantive, varied full-scroll value. | Choose one bounded content-depth seam per package. Reuse current Source/content/relationship/Place owners. Include bounded preparation recovery when needed for reliable supply; no new generator/store. One owner for shared root composition, contracts and receiving adapters. |
