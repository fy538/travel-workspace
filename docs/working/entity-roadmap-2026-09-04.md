...
- Mobile entity baseline: `95446d5a6` — canonical mock envelopes for all known
...
- Workspace entity evidence: `e4aa3b0` — roadmap, runbook, native receipt and
...
| E1/E2 | The private occurrence artifact now suppresses blocks from cancelled or archived Plans while retaining completed-trip history; PostgreSQL coverage proves cancellation removes the artifact read as well as relationship projections | `travel-agent:03f8cb219` |
| E1/E2 | Late geofence callbacks now reject cancelled or archived Plans before writing occurrence evidence; PostgreSQL coverage proves cancellation cannot resurrect presence evidence after relationship and artifact reads have been repaired | `travel-agent:108116e4e` |
| E1/E2 | The legacy in-trip relationship marker now filters cancelled or archived Plans, with PostgreSQL coverage proving compatibility consumers lose the marker after cancellation | `travel-agent:ab569c693` |
| E1/E2 | Expense evidence ingestion now uses the eligible block set, rejecting late receipts for cancelled/archived Plans or cancelled blocks; PostgreSQL coverage proves historical block IDs cannot resurrect evidence | `travel-agent:13d836ee2` |
| E1/E2 | The shared occurrence-evidence writer now locks the Plan, verifies the block belongs to the latest eligible itinerary, and rejects terminal Plans/cancelled blocks for every source (including direct photo evidence); PostgreSQL cancellation coverage exercises the guard | `travel-agent:e6bf7a75f` |
...
| E1/E10 | Experience-outcome writes and trip-artifact reads now preserve the itinerary block's canonical `EntityRef` for venue, site, and experience identities; private memory claims use that same typed subject key, including idempotent retraction readback | `travel-agent:74bac0788` |
...
- Relationship occurrence reads now exclude cancelled and archived trips;
  ...
  projector.
- Outcome identity continuity now passes the relationship suite (6 PostgreSQL
  tests plus 14 offline relationship/artifact tests): venue, site, experience
  and free-time block shapes resolve to typed refs, write responses expose the
  ref, trip artifact readback retains it, and private claims use the same
  canonical subject key. No outcome or catalog rows were backfilled.
...
- The focused mobile shared-renderer state suite passes
  ...
  without retry. The ten focused screen/data/mock suites pass 73 tests; with
  the primary save mutation suite included, the entity-focused mobile group
  passes 103 tests;
...
- The research-request lifecycle regressions pass with the shared renderer
  ...
  `__tests__/components/places/ObjectPageRebuild.test.tsx`: 7 tests total): a
  failed receipt from account/entity A is absent immediately after a mounted
  transition to account/entity B, the next request receives a fresh
...
- The venue and experience smoke suites now include deferred route-replacement
  tests for canonical plan previews; both suites pass (**38 tests** together).
  A late preview may finish on the server, but it cannot mount a review sheet,
...
- The exact pilot-focused app selection (object-page projection/rebuild,
  ...
  mock-only direct-read state harness) passes **102 tests**. The original
