...
| Normalized text/image representation | `intake_processing_runs.output`, `workers/intake_v2_jobs.py` | Deterministic normalization input for semantic interpretation | Replayable processing receipt; source-local and owner-scoped. Accepted expiry/delete cleanup clears content-bearing `output` for selected source IDs while retaining content-free run identity/status. | New-source processing copies are purged with ephemeral source bytes; any separate ordinary-history or legacy-copy retention rule remains decision-gated and must not restore released source content invisibly. |
...
| Pending Chat payload | `pending_chat_turns.payload` and pending-turn images | Bridge an admitted source into one Chat gesture before canonical message persistence | Device-scoped, stateful outbox; accepted/cancelled/expired states are reconciled before retry; a whole-submission ref also requires an active owner envelope plus one custody-valid child source at send time | Persist authored note and source refs separately; a retry reuses the gesture identity, while a later Ask gets a new identity (implemented in `9f00c92ce`, `3ff5dac62`, and `a7cbec6cf`). |
...
- `a7cbec6cf` requires submission-level pending-Chat refs to have both an active
  owner envelope and at least one custody-valid child source before prompting.
- `49ec11e21` proves PostgreSQL expiry clears both normalization and semantic
  processing-run outputs for an ephemeral source before cleanup completes.
