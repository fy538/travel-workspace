...
### 7. Hand-rolled TTL caches beside the shared primitive (B)

`core/cache/ttl_cache.py` exists to kill this pattern (17 adopters) yet ≥10 hand-rolled stores remain (`user_signals.py:72`, `user_modality_state.py:49`, `map_state/enrichment.py:51`, `location_context.py:362`, `exchange_rates.py:25`, `tool_cache.py:43`, `pick_judgment.py:194`, `compose.py:257,341`, `db/trips.py:3646`…). ~1 day. **Exempt:** `idempotency.py` (dual-TTL in-flight/completed) and `places/cache.py` (negative-TTL) don't map 1:1 — leave those two.
